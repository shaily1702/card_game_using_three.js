...
import {CARDS, rockTexture, paperTexture, scissorTexture} from './cards';
...
gltfLoader.load('./assets/table.glb', function(glb) {
...
function nextRound() {
    if(CARDS[0].name === 'hand playerCard1 paper') {
        CARDS[0].material[4].map = scissorTexture;
        CARDS[0].name = 'hand playerCard1 scissor';
        CARDS[5].material[4].map = paperTexture;
        CARDS[5].name = 'paper';
        p1.style.background = 'linear-gradient(90deg, rgba(253, 219, 0, 0.8) 0%, rgba(255, 255, 255, 0) 100%)';
        p2.style.background = 'linear-gradient(90deg, rgba(255, 0, 0, 0.8) 0%, rgba(255, 255, 255, 0) 100%)';
    } else if(CARDS[0].name === 'hand playerCard1 scissor') {
        CARDS[0].material[4].map = paperTexture;
        CARDS[0].name = 'hand playerCard1 paper';
        CARDS[5].material[4].map = scissorTexture;
        CARDS[5].name = 'scissor';
        p2.style.background = 'linear-gradient(90deg, rgba(255, 221, 0, 0.8) 0%, rgba(255, 255, 255, 0) 100%)';
        p1.style.background = 'linear-gradient(90deg, rgba(255, 0, 0, 0.8) 0%, rgba(255, 255, 255, 0) 100%)';
    }
}
...
function resetAndUpdate(side, sideText) {
    ...
    for(let i = 0; i < 5; i++) {
        ...
        if(CARDS[i].name === 'hand playerCard1 scissor' || CARDS[i].name === 'hand playerCard1 paper')
...
window.addEventListener('click', function(e) {
    ...
    if(hoveredCard && playNext && !finished) {
        ...
        .to(opponentCards[randomNumber].position, {
            ...
            onComplete: function() {
                playNext = true;
                if(hoveredCName.includes('paper') && oppCName.includes('scissor'))
                    resetAndUpdate('opponent', oScore);
                if(hoveredCName.includes('paper') && oppCName.includes('rock'))
                    resetAndUpdate('player', pScore);
                if(hoveredCName.includes('rock') && oppCName.includes('scissor'))
                    resetAndUpdate('player', pScore);
                if(hoveredCName.includes('scissor') && oppCName.includes('paper'))
                    resetAndUpdate('player', pScore);
                if(hoveredCName.includes('scissor') && oppCName.includes('rock'))
                    resetAndUpdate('opponent', oScore);
                if(hoveredCName.includes('rock') && oppCName.includes('paper'))
                    resetAndUpdate('opponent', oScore);
            }
        }, 0);
